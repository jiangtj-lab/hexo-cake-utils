...
module.exports = function (hexo, pluginDir) {
  return {
    defaultConfigFile(key, file) {
      let defaultConfig = file
        ? yaml.load(fs.readFileSync(this.getFilePath(file), "utf8"))
        : {};
      let data = hexo.locals.get("data");
      hexo.config[key] = Object.assign(
        defaultConfig,
    ...
    loadPlugin(name) {
      const plugin = hexo.resolvePlugin(name, hexo.base_dir);
      hexo
        .loadPlugin(plugin, hexo.base_dir)
        .then(() => {
